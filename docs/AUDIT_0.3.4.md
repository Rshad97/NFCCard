...
- Native ad-hoc codesign of the complete app with explicit identifier `com.rashad.nfccard`, DER entitlements and a separate jailbreak entitlement file containing TAG and `platform-application`. Signing only the Mach-O with ldid passed structural checks but failed native verification (`code object is not signed at all`), so the full bundle is now sealed and verified, including Info.plist/resources. This is still a jailbreak ad-hoc signature, not Apple provisioning. Platform status is an on-device compatibility change, not a claim that error 202 always needs it. The standard Xcode entitlement file is unchanged. No fabricated team ID, NFC-daemon hook, sandbox-disable entitlement, root helper or daemon restart is introduced.
